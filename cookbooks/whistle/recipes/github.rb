...
fs_nodes = search(:node, "roles:sip AND cluster:#{node[:cluster]}")
...
git "#{node[:whistle][:homedir]}/whistle" do
  ...
  repository "git://github.com/2600hz/whistle.git"
  user "whistle"
  group "whistle"
  action :sync
end

execute "build whistle beams" do
  command "#{node[:whistle][:homedir]}/whistle/bin/build_beams.sh"
  user "whistle"
  action :run
  environment ({'HOME' => '/opt/whistle'})
  not_if { ::FileTest.directory?("#{node[:whistle][:homedir]}/whistle/ecallmgr/ebin") }
end
